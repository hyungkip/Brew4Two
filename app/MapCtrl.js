angular.module('brew.map', ['ui.bootstrap.datetimepicker'])
	.controller('MapCtrl', function($scope) {
		...
		function createMarker(place) {
			...
			var content = '<img class="showhover"src="' + photo + '">' + '<h2>' + place.name + '</h2>' + '<p>' + place.formatted_address +
				'</p>' + '<p class="opening-hours">' + openNow + '</p>' + '<p>' + 'Rating: ' + place.rating + '</p>';
		...
		function listResults(results, status) {
			if (status === google.maps.places.PlacesServiceStatus.OK) {
				...
				for (var i = 0; i < results.length; i++) {
					...
					if (results[i].photos) {
						results[i].shopImage = results[i].photos[0].getUrl({
							'maxWidth': 500,
							'maxHeight': 500
						});
					} else {
