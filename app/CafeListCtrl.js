	...
	function($scope, $http, $window, $location) {
		...
		$scope.addNewAppointment = function(shop) {
			...
			if (!hostId) {
				...
				var formatDateandTimeProperly = function(date) {
					...
					if (time.indexOf('-') !== -1) {
						$scope.newAppointment.time = $scope.newAppointment.timeee.toLocaleTimeString(navigator.language, {
							hour: '2-digit',
							minute: '2-digit'
						});
					}
				...
				} else if (appointmentMonth < currentMonth || appointmentYear < currentYear || (appointmentMonth ===
						currentMonth && appointmentDate < currentDate)) {
					alert("Pick a time that is in the future, not the past");
				} else {
					console.log("VALIDDD");
					formatDateandTimeProperly();
		...
		$scope.requestToJoin = function(thisAppointment) {
			...
			swal({
				...
				if (isConfirm) {
					$http.post('/sendJoinRequest', {
							token: hostId,
							appointment: $scope.appointmentList[thisAppointment]
						})
